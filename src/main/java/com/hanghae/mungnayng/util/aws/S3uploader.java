...
public class S3uploader {
    ...
    public String Uploader(MultipartFile multipartFile) throws IOException {
        ...
        objectMetadata.setContentType(multipartFile.getContentType());  // ContentType을 가져와서 클라이언트에 전송할 데이터 종류(MIME-TYPE) 지정 - ex) image/jpg
        objectMetadata.setContentLength(size); // getSize()로 알아낸 파일 사이즈를 클라이언트에게 전송
        InputStream inputStream = multipartFile.getInputStream();   // 받아온 multipartFile 파라미터 값에서 실제 데이터를 읽는다(가져온다) - InputStream을 통해서는 Byte만 가져옴
        amazonS3Client.putObject(new PutObjectRequest(bucketName, fileName, inputStream, objectMetadata) // objectMetadata를 통해 실제 파일에 대한 정보를 받아오는 것
