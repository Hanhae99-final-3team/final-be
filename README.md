...
|구분|설명|
|---|---|
|문제상황|기존 상품 상세 페이지 내 조회하려는 상품이 속한 itemCategory에 등록된 상품들의 평균 가격을 불러오는 메서드는 상품 상세 조회마다 계산하게 끔 되어있었고 이러한 불필요한 반복 작업은 DB에 등록된 데이터가 많아질수록 부담스러운 작업이었기에 속히 개선 시킬 필요가 있었다.|
|문제원인|서비스에서 상품 조회가 굉장히 빈번하게 발생되고 동일한 데이터를 넘겨주면 되는 작업임에도 매번 계산을 하게 끔 설정한 것이 문제였다.|
|문제해결|우선 첫 번째로 전체 상품의 평균 가격을 계산하는 것이 아닌 해당 itemCategory에 등록된 최신 100개 상품들의 평균 가격을 계산하게 끔 수정하였다. 사용자 입장에서 평균 가격이 거래에 있어 유용한 지표로서 작용될 수 있으나 1억 건의 데이터가 존재할 때 1억 개의 상품들의 평균 가격이나 최신 100개 상품의 평균 가격이나 서비스 이용에 큰 차이를 느끼지 못할 것이라 판단하였다. 오히려 최신 100개 상품 만으로 평균 가격을 도출하는 것이 소비자 입장에서 해당 상품들의 가격 동향을 파악하기에 유용할 수 있을 것이었다.<br>두 번째로 **Spring Cache**를 도입하여 최초의 상품 조회에서만 평균 가격을 계산하고 그 뒤로 일정 시간 동안은 캐시 데이터로 저장된 평균 가격을 가져오게 끔 수정하였다. 이를 바탕으로 매 상품 조회 마다 평균 가격을 계산하던 불필요한 반복 작업을 확 줄일 수 있었다.<br>하지만 순수 Spring Cache로는 주기적인 평균 가격 값의 업데이트 불가하였다.<br>Redis Cache / EHCache 등을 추가 적용하여 정기적인 평균 가격 값의 업데이트가 가능하였으나 개발을 위해 소요되는 시간 부담이 결코 적지 않았다.<br>그러던 중 일전에 사용했던 **Scheduler**로 해결이 가능할 수 있다고 판단하였고 Scheduler는 return type이 void / parameter 값을 줄 수 없다는 단점이 있었지만 주기적으로 평균 가격 캐시만 비워주면 되었기에 Scheduler의 단점이 본 프로젝트에는 적용되지 않는다고 판단하여 최종적으로 Spring Cache + Scheduler을 적용하여 해당 메서드를 개선 시킬 수 있었다.|
|해결결과|기존 상품 조회 시 마다 반복적으로 평균 가격을 계산하는 메서드를 캐시를 활용하여 훨씬 더 효율적으로 동작하게 끔 리팩터링 할 수 있었다. |

### 2. 검색 메서드 개선 

|구분|설명|
|---|---|
|문제상황|중고거래 서비스에서 검색 기능은 매우 중요한 기능이다. 현재 구현한 검색 메서드는 현 프로젝트 수준의 DB row에서는 문제가 없을 수 있으나 서비스가 정식 론칭되어 사용자가 몰리며 상품 DB 데이터가 증가한다면 검색 기능이 심각하게 저하될 수 밖에 없었다.|
|문제원인|기존 검색 기능은 like %input%으로 구현하였었다. 이럴 경우 의도했던 검색은 가능하나 Full Table Scan으로 동작하는 것이 문제의 원인이었다.|
|문제해결|이 문제를 해결하기 위해 기존 검색 메서드가 Index를 타게 하려고 Index도 생성해보고 쿼리문도 바꿔보는 시도를 하였으나 결론적으로 like %input%는 필연적으로 Full Table Scan이 일어난다.(input%로 수정하기에는 기존 의도했던 검색이 불가하였다.)<br>검색을 개선할 수 있는 Elastic Search가 존재하였지만 이는 구현하기에 적지 않은 시간이 필요하였고 남은 프로젝트 기한 내 이를 완전히 이해하면서 구현하는 것은 불가하다고 판단되었다.<br>추가적으로 Index와 관련한 학습을 하던 중 Full Text Index에 대해서 알게 되었고 해당 Index를 사용하여 MATCH() AGAINST() 함수를 이용하면 기존의 like %input% 형태의 검색 메서드를 개선시킬 수 있다는 것을 알게 되었다.<br>JPQL에서는 MATCH() 함수를 지원하지 않기에 Native Query로도 간단하게 적용 가능하였으나 JPA의 장점을 최대한 활용하기 위하여 Custom MysqlDialect Class를 생성하여 MATCH() 함수에 대한 설정을 해주고 Querydsl을 이용하여 구현하였다.|
|해결결과|기존 like %input% 형태로 구현하였던 검색 메서드를 개선 시킬 수 있었다. 본 프로젝트 DB로는 DB row가 많지 않아 기존의 것과 속도적인 부분에서 큰 차이를 보이지는 않았으나 테스트용 DB를 만들어 데스트 해본 결과 속도적인 측면에서 확실한 성능 향상을 확인 할 수 있었다.|
